...
class Hasil extends CI_Controller {
 ...
 public function lihat() {    
	...
	public function word3() {
      ...
      $this->load->view('hasil/tb_hasil_word',$data);
		...
		public function cetak() {
		     ...
		     $data['ion1'] = $this->Hasil_model->get_hasil();	
      $this->template->display('hasil/tb_hasil_cetak',$data);
	  	$data1 = array(
	...
	function view_data(){ 
	  ...
	  $hsl2="Hasil Rapat belum diinput";
		...
		$hsl1=$hasil->hasil;
          ...
          'tanggal'=>tgl_indo($hasil->tanggal),  
                'tanggal1'=>$tgl ,
                ...
                'delete'=>anchor('hasil/delete/' . $hasil->id, '<i class="btn-sm btn-danger glyphicon glyphicon-trash" data-toggle="tooltip" title="delete"></i>'),
				     'prints'=>anchor('hasil/print1/' . $hasil->id, '<i class="btn-sm btn-danger glyphicon glyphicon-print" data-toggle="tooltip" title="print"></i>'),
      ...
      $this->Log_model->insert($data1);
        ...
        if ($row) {
            $data = array(
			   ...
			   $data['jadwal'] = $this->db->get_where('jadwal', array('nama' => $row->nama))->row_array();
            $this->load->view('hasil/tb_hasil_word1', $data);
        } else {
            $this->session->set_flashdata('message', 'Record Not Found');
            redirect(site_url('hasil'));
        }
    ...
    public function prints($id) {
        ...
        if ($row) {
			...
			$susunan=$row->susunan;
			 ...
			 $this->session->set_userdata('uraian',$uraian);   
			 $this->session->set_userdata('susunan',$susunan);   
	
		$get =$this->db->get_where("jadwal",array("nama" =>$nama))->row();
			 $pukul = $get->jam1;	
	$tanggal = $get->tanggal;
	$tanggal1 = $get->tanggal1;
	$tempat = $get->tempat;
				 $this->session->set_userdata('jam',$pukul);   
    ...
    public function _rules() {
        $this->form_validation->set_rules('hasil', 'hasil rapat', 'trim|required');
        $this->form_validation->set_rules('susunan', 'susunan rapat', 'trim|required');
        $this->form_validation->set_rules('uraian', 'uraian rapat', 'trim|required');
...
xlsWriteLabel(0,0,$judul); 
        ...
        foreach ($this->Hasil_model->get_hasil() as $data) {
			$f1=$data->tanggal1;
			$f=$data->tanggal;
 if($f1!=$f){
	 $n= $data->tanggal1;
...
if($f2==null){
	...
	$d= $data->hasil;
            ...
            xlsWriteLabel($tablebody, $kolombody++, $d);
 ...
 public function excel1() {
      ...
      $this->Log_model->insert($data1);
        $this->load->helper('exportexcel');
        $namaFile = "JadwalRapat.xls";
        $judul = "Data Jadwal Rapat";
        $tablehead = 2;
        $tablebody = 3;
        $nourut = 1;
        //penulisan header
