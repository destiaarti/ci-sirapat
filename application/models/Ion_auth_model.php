...
class Ion_auth_model extends CI_Model {
  ...
  $ip_address = $this->_prepare_ip($this->input->ip_address());
            return $this->db->insert($this->tables['tb_login_attempts'], array('ip_address' => $ip_address, 'login' => $identity,'status' => "berhasil", 'time' => time(),'datetime1' => date("Y-m-d H:i:s"),));
    ...
    public function increase_login_attempts($identity) {
        if ($this->config->item('track_login_attempts', 'ion_auth')) {
            $ip_address = $this->_prepare_ip($this->input->ip_address());
            return $this->db->insert($this->tables['tb_login_attempts'], array('ip_address' => $ip_address, 'login' => $identity,'status' => "gagal",'datetime1' => date("Y-m-d H:i:s"), 'time' => time(),));
        }
        return FALSE;
    }
   ...
   function insert($data) {
	 ...
	 $this->db->insert($this->tables['tb_users'], $data);
    }
	   function insert1($data) {
  
	 $this->db->insert($this->tables['tb_users_groups'], $data);
    }
